...
int main() {
  struct dpu_set_t set, dpu; // set : DPU set, dpu : DPU
  uint32_t checksum; 

  DPU_ASSERT(dpu_alloc(1, NULL, &set)); // Allocate 1 DPU
  DPU_ASSERT(dpu_load(set, DPU_BINARY, NULL)); // Load the binary file on the DPUs
  populate_mram(set);
